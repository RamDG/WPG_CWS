function [ P_Fi, P_SR, P_SL, xi_Fi, lambda] = generate_step( r, alpha,...
...
for i = nb_points-step+1:nb_points
    ...
    P_Fi(2,swing_foot,i) = P_Fi(2,swing_foot,i) + ...
        r*sin(alpha);
    t_down = linspace(0,time,step);
    P_Fi(3,swing_foot,i) = P_Fi(3,swing_foot,i) + ...
        mid_height-mid_height/time*t_down(i-nb_points+step);
end

% generate the backward trajectory for the support foot
% if swing_foot == 1
%     support = 2;
% else
%     support = 1;
...
% 
% for i = step+1:nb_points-step
%     P_Fi(1,support,i) = P_Fi(1,support,i) + ...
%         -(r/(T-2*time))*t_vec(i-step)*cos(alpha);
%     P_Fi(2,support,i) = P_Fi(2,support,i) + ...
%         -(r/(T-2*time))*t_vec(i-step)*sin(alpha);
% end
% 
% for i = nb_points-step+1:nb_points
